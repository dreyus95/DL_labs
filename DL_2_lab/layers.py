...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        forward_pass = np.dot(inputs, self.weights.T) + self.bias
        return forward_pass
    ...
    def backward_inputs(self, grads):
        ...
        """
        backward_inputs = np.dot(grads, self.weights)
...
def softmax(x):
    ...
    res = np.divide(exp_scores, sumexp)
    return res
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        """
        res = (y * -np.log(softmax(x))).sum(axis=1).mean()
        return res
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        return self.weight_decay * np.sum(self.weights * self.weights)
